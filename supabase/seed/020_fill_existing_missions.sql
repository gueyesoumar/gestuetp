...
BEGIN
  ...
  -- Boucle sur chaque mission PSSI-ES
  FOR v_mission IN
  ...
  LOOP
    ...
    -- Trouver le lead auditor (ou fallback sur un utilisateur du cabinet)
    ...
    -- S'assurer que le lead est membre de la mission
    ...
    -- Créer les évaluations pour tous les contrôles PSSI-ES
    FOR v_control IN
    ...
    LOOP
      ...
      -- Distribution réaliste des résultats
      ...
      ELSIF v_counter % 7 = 0 THEN
        ...
        v_findings := 'Non-conformité mineure sur ' || v_control.code || '. La mesure est partiellement mise en œuvre mais des lacunes subsistent.';
      ...
      ELSIF v_counter % 5 = 0 THEN
        ...
        v_findings := 'Observation sur ' || v_control.code || '. La mesure est en place mais des améliorations sont possibles.';
      ...
      ELSIF v_counter % 13 = 0 THEN
        ...
        v_findings := 'Point fort identifié sur ' || v_control.code || '. Mise en œuvre exemplaire et bien documentée.';
        v_recommendations := NULL;
      ELSE
        v_classification := NULL;
        v_findings := 'Conforme. La règle ' || v_control.code || ' (' || v_control.name || ') est correctement mise en œuvre.';
        v_recommendations := NULL;
      END IF;
      ...
      VALUES (
        ...
        v_lead_id,
        'approved'::public.assessment_status,
        v_findings,
        v_recommendations,
        v_classification
      );
    ...
    -- Réponses client sur les CARs non ouvertes
    ...
    -- Passer la mission en closure
    UPDATE public.missions
    SET status = 'closure'
    WHERE id = v_mission.id;

    v_mission_count := v_mission_count + 1;
    RAISE NOTICE 'Mission "%" remplie : % contrôles évalués.', v_mission.name, v_counter;
