...
| V4 | `useFieldworkState.ts` + `useMissionDocuments.ts` + `useCampaignDetail.ts` + `useEntityDetail.ts` + `useClientEvidenceUpload.ts` | 6 erreurs TypeScript régressives introduites pendant la remédiation (E7 avait retiré l'import `supabase` mais `saveAssessment` l'utilisait encore → **ReferenceError runtime** ; `.catch()` sur un `PromiseLike` ; `.abortSignal()` après `.single()` ; type `RefObject`) | ✅ corrigé ; `tsc -b` repasse à **0 erreur**, build OK |
