...
import { ModuleAdoption } from '../../features/admin/dashboard/ModuleAdoption'
import { AttentionFeed, type FeedItem } from '../../features/admin/dashboard/AttentionFeed'
...
export function AdminDashboardPage() {
  ...
  const items: FeedItem[] = [
    ...stats.alerts.map((a): FeedItem => ({ severity: a.kind === 'red' ? 'crit' : a.kind, title: a.message })),
    ...stats.signals.map((s): FeedItem => ({ severity: s.severity, title: s.title, detail: s.detail })),
  ]
    ...
    <div className="px-7 py-6">
      <div className="flex items-end justify-between gap-4 mb-6 flex-wrap">
        <div>
          ...
          <h1 className="text-xl font-bold text-gray-900">Bonjour {profile?.first_name ?? ''} — voici ce qui compte aujourd&apos;hui</h1>
          <p className="text-[12.5px] text-gray-500 mt-1">{today} · {crit} signal(aux) critique(s), {warn} à surveiller.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-4">
        ...
        <div className="flex flex-col gap-4">
          <ModuleAdoption data={stats.module_adoption} totalOrgs={stats.cabinets_total} />
