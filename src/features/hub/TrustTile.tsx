...
export function TrustTile({ tile }: { tile: TrustTileData }): JSX.Element {
  ...
  return (
    <div className="rounded-2xl border border-[rgb(var(--hub-fg)/0.1)] bg-[rgb(var(--hub-fg)/0.04)] backdrop-blur-sm p-4 flex items-center gap-4">
      <svg viewBox="0 0 56 56" className="w-[52px] h-[52px] shrink-0" aria-hidden="true">
        ...
        <text x="28" y="32" textAnchor="middle" className="fill-[rgb(var(--hub-fg))]" style={{ fontSize: 15, fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>
          {tile.score === null ? '—' : tile.score}
        </text>
      </svg>
      <div className="min-w-0">
        <div className="text-[14px] font-bold text-[rgb(var(--hub-fg))] truncate">{tile.name}</div>
