...
// (lignes compactes) + 2 facteurs. Clients/Groupe : tuiles-score par entité.
// Placé à côté de l'orbite (pas dessous) pour lui laisser toute la hauteur.

function DimRow({ dim, factor = false }: { dim: DimScore; factor?: boolean }): JSX.Element {
      ...
      <div className="min-h-0 flex-1 overflow-y-auto pr-1">
        {mode === 'self' ? (
          <>
            ...
            {factors.length > 0 && (
              <div className="mt-3 font-mono text-[8px] uppercase tracking-[0.12em] text-white/40">Facteurs transverses</div>
            )}
            {factors.map((d) => <DimRow key={d.key} dim={d} factor />)}
          </>
        ) : tiles.length === 0 ? (
