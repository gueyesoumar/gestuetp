...
function DimRow({ dim }: { dim: DimScore }): JSX.Element {
  ...
  return (
    <div className="grid grid-cols-[1fr_auto] items-center gap-x-3 border-t border-[rgb(var(--hub-fg)/0.1)] py-2.5">
      <div className="min-w-0">
        <div className="truncate text-[12.5px] font-semibold text-[rgb(var(--hub-fg))]">{SCORE_DIMENSION_LABELS[dim.key]}</div>
        <div className="mt-0.5 font-mono text-[8.5px] uppercase tracking-[0.06em] text-[rgb(var(--hub-fg)/0.4)]">
      ...
      <div className="col-span-2 mt-1.5 h-[5px] overflow-hidden rounded-full bg-[rgb(var(--hub-fg)/0.1)]">
...
function FactorRow({ factor }: { factor: FactorScore }): JSX.Element {
  ...
  return (
    <div className="grid grid-cols-[1fr_auto] items-center gap-x-3 border-t border-[rgb(var(--hub-fg)/0.1)] py-2.5">
      <div className="min-w-0">
        <div className="truncate text-[12.5px] font-semibold text-[rgb(var(--hub-fg))]">{SCORE_FACTOR_LABELS[factor.key]}</div>
        <div className="mt-0.5 font-mono text-[8.5px] uppercase tracking-[0.06em] text-[rgb(var(--hub-fg)/0.4)]">
...
export function HubSidePanel({ mode, title, axes = [], factors = [], tiles = [], emptyLabel }: HubSidePanelProps): JSX.Element {
  return (
    <aside className="flex min-h-0 w-full flex-col md:w-[340px] md:shrink-0 md:border-l md:border-[rgb(var(--hub-fg)/0.1)] md:pl-6">
      <div className="mb-2 shrink-0 font-mono text-[10px] uppercase tracking-[0.14em] text-[rgb(var(--hub-fg)/0.4)]">{title}</div>
      <div className="min-h-0 flex-1 overflow-y-auto pr-1">
        {mode === 'self' ? (
          <>
            {axes.map((d) => <DimRow key={d.key} dim={d} />)}
            {factors.length > 0 && (
              <div className="mt-3 font-mono text-[8px] uppercase tracking-[0.12em] text-[rgb(var(--hub-fg)/0.4)]">
          ...
          <p className="text-[13px] text-[rgb(var(--hub-fg)/0.4)]">{emptyLabel}</p>
        ) : (
