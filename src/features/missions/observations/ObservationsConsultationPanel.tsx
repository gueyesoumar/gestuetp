...
import { useAssessmentObservations } from './useAssessmentObservations'
import { supabase } from '../../../lib/supabase'
import { LoadingSpinner } from '../../../components/ui/LoadingSpinner'
import { ErrorAlert } from '../../../components/ui/ErrorAlert'
import { FindingsList } from '../fieldwork/findings/FindingsList'
import type { ObservationWithAuthor } from './useAssessmentObservations'
import type { AssessmentFinding } from '../../../types/database.types'

interface ObservationsConsultationPanelProps {
  missionId: string
  heading: string
  subheading: string
  emptyLabel?: string
...
/**
 * Panneau contrôleur de consultation : liste les observations NON BLOQUANTES
 * (client audité / assujetti) sur les constats et permet d'y répondre en
 * décidant « modifié » ou « conservé ». Réutilisé par la Validation client
 * (moteur audit) et la Revue (moteur controle — remarques de l'assujetti).
 */
export function ObservationsConsultationPanel({ missionId, heading, subheading, emptyLabel }: ObservationsConsultationPanelProps): JSX.Element {
  const { observations, loading, error, submitResponse, submitting, refetch } = useAssessmentObservations(missionId)
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-[13px] text-gray-500 mt-0.5">{subheading}</p>
      </div>

      <div className="flex gap-2">
        <FilterBtn active={filter === 'pending'} onClick={() => setFilter('pending')} label="En attente" count={pending.length} highlight />
        <FilterBtn active={filter === 'responded'} onClick={() => setFilter('responded')} label="Répondues" count={responded.length} />
      ...
      {filtered.length === 0 ? (
        <div className="bg-white border border-gray-200 rounded-xl p-12 text-center">
          ...
          <p className="text-sm text-gray-400">
            {filter === 'pending' && 'Aucune remarque en attente.'}
            {filter === 'responded' && 'Aucune remarque répondue.'}
            {filter === 'all' && (emptyLabel ?? 'Aucune remarque déposée pour le moment.')}
          </p>
        </div>
      ) : (
          ...
          {filtered.map((obs) => (
            <ObservationRow
              ...
              onSubmit={async (responseText, action) => {
                ...
                if (ok) { setExpandedId(null); refetch() }
                return ok
              }}
  ...
  return (
    <button
      ...
      className={`flex items-center gap-1.5 px-3 py-1.5 text-[12px] font-semibold rounded-lg border transition-colors ${
        active ? 'bg-forest-700 text-white border-forest-700' : 'bg-white text-gray-700 border-gray-200 hover:border-forest-300'
      }`}
    >
      {label}
      <span className={`text-[10px] px-1.5 rounded-full ${active ? 'bg-white/20' : highlight && count > 0 ? 'bg-gold-100 text-gold-600' : 'bg-gray-100 text-gray-500'}`}>{count}</span>
    </button>
  )
}
...
function ObservationRow({ observation, expanded, onToggle, onSubmit, submitting }: {
  ...
  const loadAssessment = async (): Promise<void> => {
    ...
    const { data } = await supabase
      ...
      .select('id, control:controls(code, name)')
  ...
  return (
    <div className={`bg-white border rounded-xl overflow-hidden ${isResponded ? 'border-gray-200' : 'border-gold-200 border-l-[3px] border-l-gold-500'}`}>
      <button
        onClick={() => { onToggle(); if (!expanded) loadAssessment() }}
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="text-[12px] font-semibold text-gray-900">{observation.authorName ?? 'Auteur'}</span>
        ...
        {isResponded ? (
          observation.response_action === 'modified' ? (
            <span className="text-[10px] font-medium text-forest-700 bg-forest-50 px-2 py-0.5 rounded-full inline-flex items-center gap-1 shrink-0"><FileEdit size={10} /> Modifié</span>
          ) : (
            <span className="text-[10px] font-medium text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full inline-flex items-center gap-1 shrink-0"><CheckCircle2 size={10} /> Conservé</span>
          )
        ) : (
          <span className="text-[10px] font-medium text-gold-600 bg-gold-50 px-2 py-0.5 rounded-full inline-flex items-center gap-1 shrink-0"><Clock size={10} /> En attente</span>
        )}
      </button>
        ...
        <div className="border-t border-gray-200 bg-gray-50">
          {assessment && (
            <div className="px-4 py-3 border-b border-gray-200 bg-white">
              <div className="flex items-center gap-2 mb-2">
                <span className="font-mono text-[11px] font-semibold text-forest-700 bg-forest-50 px-1.5 py-0.5 rounded">{assessment.controlCode}</span>
                <span className="text-[13px] font-semibold">{assessment.controlName}</span>
              </div>
              <FindingsList findings={assessment.findings} emptyMessage="Aucun constat enregistré sur ce contrôle." density="compact" />
            </div>
          )}

          <div className="px-4 py-3 bg-forest-50 border-b border-forest-100">
            <div className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Remarque</div>
          ...
          {isResponded ? (
            <div className="px-4 py-3 bg-gold-50">
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <span className="ml-auto text-[10px] font-medium px-2 py-0.5 rounded-full inline-flex items-center gap-1"
                  style={{ background: observation.response_action === 'modified' ? '#D1FAE5' : '#F3F4F6', color: observation.response_action === 'modified' ? '#059669' : '#6B7280' }}>
                  {observation.response_action === 'modified' ? <><FileEdit size={10} /> Constat modifié</> : <><CheckCircle2 size={10} /> Constat conservé</>}
                </span>
              </div>
            ...
            <div className="px-4 py-3">
              <div className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-2">Votre réponse</div>
              <textarea
                value={responseText}
                onChange={(e) => setResponseText(e.target.value)}
                placeholder="Répondre et expliquer votre décision..."
                rows={3}
              ...
              <div className="flex items-center justify-between mt-3">
                <p className="text-[10px] text-gray-400">Votre réponse sera visible immédiatement.</p>
                <div className="flex gap-2">
                  <button onClick={() => onSubmit(responseText, 'kept')} disabled={!responseText.trim() || submitting}
                    ...
                    <CheckCircle2 size={12} /> Conserver le constat
                  </button>
