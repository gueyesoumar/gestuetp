...
interface Props {
  subsidiaries?: Assujetti[]
  /** Si fourni : entité imposée (self-déclaration assujetti), pas de choix. */
  fixedEntity?: { id: string; name?: string }
  onClose: () => void
  onSuccess: () => void
}

/** Déclaration d'un incident cyber (régulateur : choix de l'assujetti ; assujetti : sa propre org). */
export function IncidentDeclareModal({ subsidiaries = [], fixedEntity, onClose, onSuccess }: Props): JSX.Element {
  const { declare, busy } = useDeclareIncident()
  const [error, setError] = useState<string | null>(null)
  const [entityId, setEntityId] = useState(fixedEntity?.id ?? subsidiaries[0]?.id ?? '')
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-3">
          ...
          {fixedEntity ? (
            fixedEntity.name && (
              <div>
                <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Organisation</label>
                <p className="mt-1 text-[13px] font-semibold text-gray-800">{fixedEntity.name}</p>
              </div>
            )
          ) : (
            <div>
        ...
        <div className="p-4 border-t border-gray-200 flex justify-end gap-2">
          ...
          <button onClick={submit} disabled={busy || !entityId} className="px-4 py-2 bg-forest-700 text-white rounded-lg text-xs font-semibold hover:bg-forest-900 disabled:opacity-50">
