...
export function PlansMatrixView({ plans, featuresByPlan, onSetFeatures }: PlansMatrixViewProps): JSX.Element {
  ...
  const handleToggle = async (planId: string, flagId: string): Promise<void> => {
    if (!reason.trim()) {
    ...
    <div className="bg-gold-50 border border-gold-300 rounded-lg p-3 flex items-start gap-3">
      ...
      <div className="flex-1">
        ...
        <p className="text-[10.5px] text-gold-700 mt-1">
          Ce motif sera tracé dans l&apos;audit log pour chaque modification de cellule.
        </p>
      </div>
    </div>
  )
...
interface CategorySectionProps {
  ...
  plans: AdminPlan[]
  featuresByPlan: Map<string, Set<string>>
  onToggle: (planId: string, flagId: string) => void
  disabled: boolean
  savingPlans: Set<string>
}

function CategorySection({ label, items, plans, featuresByPlan, onToggle, disabled, savingPlans }: CategorySectionProps): JSX.Element {
  return (
    <>
          ...
          {plans.map((plan) => {
            ...
            return (
              <td key={plan.id} className={`text-center ${plan.is_default ? 'bg-gold-50/30' : ''}`}>
                <Cell on={isOn} disabled={cellDisabled} maturity={item.maturity} onClick={() => onToggle(plan.id, item.id)} />
              </td>
            )
          })}
        </tr>
...
function Legend(): JSX.Element {
  return (
    <div className="flex flex-wrap items-center gap-4 text-[11px] text-gray-500 px-1">
      ...
      <span className="ml-auto text-[10.5px] text-gray-400">Clic pour basculer · auto-save</span>
    </div>
  )
}
