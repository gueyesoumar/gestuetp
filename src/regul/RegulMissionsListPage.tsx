...
export function RegulMissionsListPage(): JSX.Element {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="mt-1 text-[13px] text-gray-500">
            {missions.length === 0 ? 'Aucune mission de contrôle.' : `${missions.length} mission${missions.length > 1 ? 's' : ''}`}
          </p>
        </div>
      ...
      {missions.length === 0 ? (
        <EmptyState title="Aucune mission de contrôle" description="Planifiez votre premier audit sur un assujetti avec le bouton ci-dessus." />
      ) : (
